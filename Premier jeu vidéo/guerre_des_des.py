...
class GuerreDesDes:
    ...
    def attaque_succes(self, attaquant, defenseur):
        ...
        des_qui_reste.append(attaquant.des[0])
        des_qui_transfer = attaquant.des[1:]
        attaquant.remplacer_des(des_qui_reste)
        defenseur.remplacer_des(des_qui_transfer)
    ...
    def fin_du_tour(self, joueur):
        ...
        plus_grand_territoire = self.carte.taille_plus_grand_territoire(joueur)
        de_creation_counter = 0
        liste_nouveau_des = []
        while de_creation_counter < plus_grand_territoire:
            liste_nouveau_des.append(De(6))
            de_creation_counter += 1
        joueur.ajouter_n_des(liste_nouveau_des)
    ...
    def partie_terminee(self):
        ...
        Returns:
            bool: True si il y a un joueur gagnant. (Correction, confirmé par l'enseignant)
        """
        # VOTRE CODE ICI

        if self.determiner_gagnant():
            return True
    ...
    def conquete(self, joueur):
        ...
        if len(self.carte.cases) == len(self.carte.obtenir_cases_joueur(joueur)):
            joueur_gagnant = True
        return joueur_gagnant

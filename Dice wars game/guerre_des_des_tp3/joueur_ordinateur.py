...
class JoueurOrdinateur(Joueur):
    ...
    def filtrer_nb_des(self, cases, valeurs_acceptees):
        ...
        liste_cases = list(cases.values())
        cases_acceptees = {}
        for case in liste_cases:
            de_par_case = case.nombre_de_des()
            if de_par_case in valeurs_acceptees:
                cases_acceptees[case.coordonnees] = case
    ...
    def trouver_nb_des_optimal(self, cases, minimum=False):
        ...
        case_valeur_max = 0
        case_valeur_min = 9
        case_selectionner = None
        for case in cases:
            if minimum is False:
                if case_valeur_max < cases[case].nombre_de_des():
                    case_valeur_max = cases[case].nombre_de_des()
                    case_selectionner = cases[case]
            elif minimum is True:
                if case_valeur_min > cases[case].nombre_de_des():
                    case_valeur_min = cases[case].nombre_de_des()
                    case_selectionner = cases[case]
        return case_selectionner

...
from tkinter import IntVar, Radiobutton, Button, Label, Entry, Frame, filedialog, END, messagebox, RIDGE, Toplevel, Tk
...
class FrameCarte(Frame):
    def __init__(self, parent):
        ...
        self.radio_generer = Radiobutton(self, text="Générer une carte", variable=self.choix_mode, value=1, command=self.selection_mode, background="black", foreground="red", font=("medieval", 16))
        self.radio_importer = Radiobutton(self, text="Importer une carte", variable=self.choix_mode, value=2, command=self.selection_mode, background="black", foreground="red", font=("medieval", 16))
        self.frame_generer = Frame(self, background="black")
        self.label_hauteur = Label(self.frame_generer, text="Hauteur: ", background="black", foreground="red", font=("medieval", 14))
        self.entry_hauteur = Entry(self.frame_generer, width=5, font=("medieval", 12))
        self.label_largeur = Label(self.frame_generer, text="Largeur: ", background="black", foreground="red", font=("medieval", 14))
        self.entry_largeur = Entry(self.frame_generer, width=5, font=("medieval", 12))
        self.label_nb_trous = Label(self.frame_generer, text="Nombre de trous: ", background="black", foreground="red", font=("medieval", 14))
        self.entry_nb_trous = Entry(self.frame_generer, width=5, font=("medieval", 12))
        ...
        self.frame_importer = Frame(self, background="black")
        self.entry_fichier = Entry(self.frame_importer, text="")
        self.bouton_fichier = Button(self.frame_importer, text="Sélectionner...", command=self.choisir_fichier, background="black", foreground="red", font=("medieval", 11))
    ...
    def obtenir_carte(self):
        ...
        if self.choix_mode.get() == 1:
            ...
            return CarteAutogeneree(hauteur, largeur, nb_trous)  # gerer les exeption de 0 and stuff si j'ai le temps
...
class FrameJoueurs(Frame):
    def __init__(self, parent):
        ...
        """
        super().__init__(parent, borderwidth=1, relief=RIDGE)
        label_joueurs = Label(self, text="Sélectionnez les joueurs", font="medieval, 16", foreground="red", background="black")
        label_joueurs.grid(row=0, column=0, padx=10, pady=10)
        self.boutons_joueur = []
        self.couleurs = list(couleurs_interface.keys())
        frame_boutons = Frame(self, background="black")
        frame_boutons.grid(row=1, column=0)
        self.configure(background="black")
        for i in range(6):
            bouton_joueur = Button(frame_boutons, text="Inactif", width=8, font=("medieval", 15, "bold"),
...
class FenetreIntroduction(Toplevel):
    def __init__(self, parent):
        ...
        self.carte = None
        self.joueurs = None
        self.label_introduction = Label(self, text="Welcome to die", background="black", foreground="red", font=("medieval", 18))
        self.label_introduction.grid(row=0, column=0, padx=10, pady=10)
        self.frame_frame = Frame(self, background="black")
        ...
        self.bouton_commencer = Button(self, text="START!", command=self.commencer, font=("medieval", 18), background="red", foreground="black")

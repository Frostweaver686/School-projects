...
"""
import sys
from tkinter import Tk, Frame, Button, Label, Toplevel, Scale, DoubleVar
...
class FrameAttaque(Frame):
    def __init__(self, parent):
        ...
        self.label_joueur_attaque = Label(self, text="", background="black", font=("medieval", 24))
        self.label_force_attaque = Label(self, text="", background="black", font=("medieval", 24))
        self.label_joueur_defense = Label(self, text="", background="black", font=("medieval", 24))
        self.label_force_defense = Label(self, text="", background="black", font=("medieval", 24))
...
class FrameJoueurActif(Frame):
    def __init__(self, parent):
        ...
        self.label_nom_joueur = Label(self, text="Joueur actif", font=("medieval", 20, "bold"), bg="black")
        self.label_des_surplus_fixe = Label(self, text="Dés en surplus:", font=("medieval", 14, "bold"), fg="white", bg="black" )
        self.label_des_surplus_variable = Label(self, text="0", font=("medieval", 14), fg="white", bg="black")
        self.bouton = Button(self, text="-", width=22, height=2, font=("medieval", 14), foreground="black", background="dark grey", command=self.clic_bouton)
        self.scale_button = Scale(self, variable=DoubleVar(), length=200, orient="horizontal", from_=10, to=500, foreground="white", background="black", command=self.varier_vitesse)
        self.scale_button.grid(row=3, column=2, pady=15)
        ...
        self.label_text_scale = Label(self, text="\nVitesse du A.I.: ", font=("medieval", 12, "bold"), background="black", foreground="white")
        self.label_text_scale.grid(row=3, column=1, pady=10)
        self.configure(background="black")

    def varier_vitesse(self, scale_value):
        """
        Cette fonction reçoit la valeur de scale_button de la classe FrameJoueurActif et
        retourne la valeur dans la variable globale TEMPS_ATTENTE.

        Args:
            La valeur ou le scale_button s'est arrêté.

        """
        global TEMPS_ATTENTE
        TEMPS_ATTENTE = scale_value
...
class FenetrePrincipale(Tk):
    def __init__(self):
        ...
        coord_x = (screen_width / 2) - (window_width / 2)
        coord_y = (screen_height / 2) - (window_height / 2)
        self.geometry("{}x{}+{}+{}".format(window_width, window_height, int(coord_x), int(coord_y)))
        self.resizable(True, True)
        self.columnconfigure(2, weight=1)
        self.rowconfigure(2, weight=1)
        self.configure(background="black")
        self.title("Dice Nukem: Dice of Steel")
        self.label_bienvenue = Label(text="Dice Nukem", font=("medieval", 140), fg="red", bg="black")
        self.label_creator_name = Label(text="Made by Frostweaver 2022®", font=("3D", 15), fg="red", bg="black")
        self.bouton_commencer = Button(text="LAUNCH", width=15, height=3, font=("medieval", 20), foreground="black", background="red", command=self.lancer_fenetre_introduction)
        self.label_bienvenue.place(relx=0.5, rely=0.1, anchor="n")
        self.bouton_commencer.place(relx=0.5, rely=0.7, anchor="center")
        self.label_creator_name.grid(row=3, column=3, padx=20, pady=10)
        self.label_intro = Label(text="It's time to kick ass and chew bubble gum... and I'm all outta gum", font=("medieval", 30), fg="red", bg="black")
        self.label_intro.place(relx=0.5, rely=0.55, anchor="center")
        self.iconbitmap(r"C:\Users\the_e\Desktop\IFT-1004\TP4\Images and others\Dn-logo.ico")
    ...
    def demarrer(self, carte, joueurs):
        ...
        self.frame_attaque.grid(row=0, column=2, padx=10, pady=10)
        ...
        self.canvas_carte.joueur_actif = self.joueur_actuel
    ...
    def afficher_attaque(self, joueur_attaque, force_attaque, joueur_defense, force_defense, suite):
        ...
        if self.est_joueur_ordi():
            ...
            self.after(200, lambda: self.redessiner(suite))
    ...
    def choice_replay(self, option):
        """
        Cette fonction reçoit en argument le choix du joueur: yes pour recommencer une partie.
        No pour fermer le jeu.

        Args:
            Yes ou No de afficher_gagnant

        """
        replay_window.destroy()
        if option == "yes":
            FenetrePrincipale.destroy(self)
    ...
    def afficher_gagnant(self):
        ...
        replay_window = Toplevel(self)
        replay_window.title("Victory!!! Player " + couleurs_interface[gagnant.couleur])
        window_width = 500
        window_height = 205
        screen_width = Tk.winfo_screenwidth(self)
        screen_height = Tk.winfo_screenheight(self)
        coord_x = (screen_width / 2) - (window_width / 2)
        coord_y = (screen_height / 2) - (window_height / 2)
        replay_window.geometry("{}x{}+{}+{}".format(window_width, window_height, int(coord_x), int(coord_y)))
        replay_window.config(bg="black")
        victory_quote = Label(replay_window, text="Hail to the king! Player " + couleurs_interface[gagnant.couleur], foreground=gagnant.couleur, background="black", font=("medieval", 22))
        victory_quote.pack(pady=10)
        replay_label = Label(replay_window, text="Would you like to play again?", foreground="red", background="black", font=("medieval", 25))
        replay_label.pack(pady=20)
        replay_window_frame = Frame(replay_window, bg="green")
        replay_window_frame.pack(pady=5)
        yes_button = Button(replay_window_frame, text="Yes", fg="black", bg="red", font=("medieval", 17), command=lambda: self.choice_replay("yes"))
        yes_button.grid(row=0, column=1)
        no_button = Button(replay_window_frame, text="No", fg="red", bg="black", font=("medieval", 17), command=lambda: self.choice_replay("no"))
        no_button.grid(row=0, column=2)
        replay_window.iconbitmap(r"C:\Users\the_e\Desktop\IFT-1004\TP4\Images and others\Dn-logo.ico")
        self.canvas_carte.permettre_clics(None)
